...
export function ProductCollectionsView() {
  ...
  const collectionProducts = (data?.products || []).filter((product: any) => product.type === 'collection')

  // Apply client-side filters for rating and featured
  const filteredProducts = collectionProducts.filter((product: any) => {
  ...
  const handleMultiDelete = async (products: any[]) => {
    try {
      ...
      toast.success(`${products.length} collections deleted successfully`)
      refetch()
    } catch (error) {
  ...
  const openCreateDialog = () => {
    // Set editingProduct to null but pass type via a different mechanism
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search collections by name, SKU, or description..."
            value={search}
      ...
      <Sheet open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <SheetContent className="w-full sm:!w-[90vw] sm:!max-w-none sm:min-w-[90vw]">
          <SheetHeader className="px-4 sm:px-8">
            <SheetTitle>{editingProduct?.id ? "Edit Collection" : "Create New Collection"}</SheetTitle>
            <SheetDescription>
              {editingProduct?.id 
                ? "Update collection information and 3D models" 
                : "Create a new product collection with multiple 3D models for a complete outfit"}
            </SheetDescription>
          </SheetHeader>
