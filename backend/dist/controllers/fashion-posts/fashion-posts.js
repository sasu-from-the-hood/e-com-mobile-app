...
import { fashionPosts, postItems, postTextElements, postLikes, postSaves, userFollows, user, products, } from '../../database/schema/index.js';
...
export const createPost = jwtProtectedProcedure
    ...
    if (isUpdate) {
        ...
        await db
            ...
            .where(and(eq(fashionPosts.id, postId), eq(fashionPosts.userId, userId)));
    ...
    if (input.items.length > 0) {
        await db.insert(postItems).values(input.items.map(item => ({
    ...
    if (input.textElements.length > 0) {
        await db.insert(postTextElements).values(input.textElements.map(text => ({
            id: crypto.randomUUID(),
            postId,
...
export const getFeedPosts = jwtProtectedProcedure
    ...
    const userEngagement = await db
        .select({
        followingCount: sql `COUNT(DISTINCT ${userFollows.followingId})`.as('followingCount'),
        postsCount: sql `COUNT(DISTINCT ${fashionPosts.id})`.as('postsCount'),
        likesGiven: sql `COUNT(DISTINCT ${postLikes.id})`.as('likesGiven'),
    })
        .from(user)
        ...
        if (followingIdsList.length > 0) {
            ...
            const followedPosts = await db
              ...
              ${fashionPosts.viewsCount} * 0.1
                ...
                .where(and(eq(fashionPosts.isPublished, true), inArray(fashionPosts.userId, followingIdsList), sql `${fashionPosts.userId} != ${userId}` // Exclude own posts
            ))
            ...
            const trendingPosts = await db
              ...
              ${fashionPosts.viewsCount} * 0.1
                ...
                .from(fashionPosts)
                .leftJoin(user, eq(fashionPosts.userId, user.id))
                .where(and(eq(fashionPosts.isPublished, true), sql `${fashionPosts.userId} != ${userId}`, // Exclude own posts
    ...
    const userLikes = postIds.length > 0
        ? await db.select().from(postLikes).where(and(inArray(postLikes.postId, postIds), eq(postLikes.userId, userId)))
        : [];
    const userSaves = postIds.length > 0
        ? await db.select().from(postSaves).where(and(inArray(postSaves.postId, postIds), eq(postSaves.userId, userId)))
        : [];
    ...
    const userFollowing = authorIds.length > 0
        ? await db.select().from(userFollows).where(and(eq(userFollows.followerId, userId), inArray(userFollows.followingId, authorIds)))
        : [];
...
export const getDraftPosts = jwtProtectedProcedure
    ...
    const posts = await db
        .select()
        .from(fashionPosts)
        .where(and(eq(fashionPosts.userId, userId), eq(fashionPosts.isDraft, true)))
...
export const deletePost = jwtProtectedProcedure
    ...
    // Delete post
    await db.delete(fashionPosts).where(and(eq(fashionPosts.id, postId), eq(fashionPosts.userId, userId)));
    return { success: true };
});
