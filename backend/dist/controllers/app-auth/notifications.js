...
export const getUnreadCount = jwtProtectedProcedure
    ...
    const unreadNotifications = await db
        .select()
        .from(notifications)
        .where(and(eq(notifications.userId, context.user.id), eq(notifications.read, false)));
...
export const markAsRead = jwtProtectedProcedure
    ...
    return await db
        ...
        .set({ read: true })
        .where(and(eq(notifications.id, input), eq(notifications.userId, context.user.id)));
});
...
export async function createNotification({ userId, title, message, type = 'info', category = 'general' }) {
