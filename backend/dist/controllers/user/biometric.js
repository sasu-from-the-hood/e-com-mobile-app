...
function verifySignature(challenge, signature, secret) {
    try {
        ...
        endpoints: {
            ...
            removeBiometric: createAuthEndpoint('/biometric/remove', {
                ...
                await db.delete(passkey)
                    .where(and(eq(passkey.userId, userId), eq(passkey.name, 'Touch ID')));
                return ctx.json({ success: true });
            }),
            checkBiometric: createAuthEndpoint('/biometric/check', {
                ...
                const result = await db.select()
                    .from(passkey)
                    .where(and(eq(passkey.userId, userId), eq(passkey.name, 'Touch ID'), eq(passkey.credentialID, hashedDeviceId)))
                    .limit(1);
            ...
            generateChallenge: createAuthEndpoint('/biometric/challenge', {
                ...
                const result = await db.select()
                    .from(passkey)
                    .where(and(eq(passkey.name, 'Touch ID'), eq(passkey.credentialID, hashedDeviceId)))
                    .limit(1);
                if (result.length === 0) {
