...
import { products, userInteractions, searchHistory } from '../../database/schema/index.js';
...
export const getRecommendations = os
    ...
    if (userHistory.length === 0) {
        ...
        const popular = await db
            .select()
            .from(products)
            .where(and(eq(products.isActive, true), eq(products.inStock, true), newArrivalIds.length > 0 ? sql `${products.id} NOT IN (${newArrivalIds.map(() => '?').join(',')})` : sql `1=1`))
            .orderBy(desc(sql `(
            ${products.rating} * 0.4 + 
    ...
    const recommendations = await db
        .select()
        .from(products)
        .where(and(eq(products.isActive, true), eq(products.inStock, true), excludeIds.length > 0 ? sql `${products.id} NOT IN (${excludeIds.map(() => '?').join(',')})` : sql `1=1`))
        .orderBy(desc(sql `(
