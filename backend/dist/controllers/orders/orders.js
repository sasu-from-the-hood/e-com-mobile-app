...
export const createOrder = jwtProtectedProcedure
    ...
    try {
        ...
        const total = selectedCart.reduce((sum, item) => sum + (parseFloat(item.products.price.toString()) * item.cart_items.quantity), 0);
        ...
        for (const item of selectedCart) {
            ...
            const totalStock = Object.values(variantStock).reduce((sum, stock) => sum + (Number(stock) || 0), 0);
        ...
        if (selectedIds.length > 0) {
            ...
            await db.delete(cartItems).where(and(eq(cartItems.userId, context.user.id), inArray(cartItems.id, selectedIds)));
        }
