    ...
    try {
        ...
        return result.map(item => {
            ...
            const colorImages = typeof item.productColorImages === 'string'
                ...
                : (item.productColorImages || {});
            const selectedColorImages = item.color && colorImages[item.color] ? colorImages[item.color] : [];
            const allColorImages = Object.values(colorImages);
            const firstColorImages = allColorImages.length > 0 && allColorImages[0] ? allColorImages[0] : [];
...
export const updateCart = os
    ...
    return await db
        ...
        .where(and(eq(cartItems.id, input.id), eq(cartItems.userId, context.user.id)));
});
export const removeFromCart = os
    ...
    return await db
        .delete(cartItems)
        .where(and(eq(cartItems.id, input), eq(cartItems.userId, context.user.id)));
});
...
export const getCartTotal = os
    ...
    const result = await db
        ...
        .from(cartItems)
        .innerJoin(products, eq(cartItems.productId, products.id))
        .where(and(eq(cartItems.userId, context.user.id), eq(cartItems.selected, 'true')));
