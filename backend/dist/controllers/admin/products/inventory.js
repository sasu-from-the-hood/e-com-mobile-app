...
import { inventoryTransactions, stockAlerts, products, productVariants } from '../../../database/schema/index.js';
    ...
    if (input.includeVariants) {
        ...
        if (productIds.length > 0) {
            variantsData = await db
                ...
                .from(productVariants)
                .where(and(eq(productVariants.isActive, true), sql `${productVariants.productId} IN (${productIds.join(',')})`));
        }
    }
    // Calculate totals
