...
export const getDashboardStats = os
    ...
    try {
        ...
        const monthlySales = await db.select({
            month: sql `DATE_FORMAT(${orders.createdAt}, '%b')`,
            sales: sql `COALESCE(SUM(${orders.total}), 0)`,
            orders: sql `COUNT(*)`
        })
            .from(orders)
            .where(gte(orders.createdAt, sql `DATE_SUB(NOW(), INTERVAL 6 MONTH)`))
            .groupBy(sql `YEAR(${orders.createdAt})`, sql `MONTH(${orders.createdAt})`, sql `DATE_FORMAT(${orders.createdAt}, '%b')`)
            .orderBy(sql `YEAR(${orders.createdAt})`, sql `MONTH(${orders.createdAt})`)
            .limit(6);
        return {
            stats: {
