...
export const getDeliveryBoyStats = adminProcedure
    ...
    try {
        const [stats] = await db
            .select({
            total: sql `CAST(COUNT(*) AS UNSIGNED)`,
            active: sql `CAST(SUM(CASE WHEN ${deliveryBoys.isActive} = true THEN 1 ELSE 0 END) AS UNSIGNED)`,
            available: sql `CAST(SUM(CASE WHEN ${deliveryBoys.isAvailable} = true THEN 1 ELSE 0 END) AS UNSIGNED)`,
            onDelivery: sql `CAST(SUM(CASE WHEN ${deliveryBoys.currentAssignedOrders} > 0 THEN 1 ELSE 0 END) AS UNSIGNED)`,
        })
            .from(deliveryBoys);
        return {
