  ...
  .handler(async () => {
    const [stats] = await db
      .select({
        ...
        active: sql<number>`SUM(CASE WHEN ${deliveryBoys.isActive} = 1 THEN 1 ELSE 0 END)`,
        available: sql<number>`SUM(CASE WHEN ${deliveryBoys.isAvailable} = 1 THEN 1 ELSE 0 END)`,
        onDelivery: sql<number>`SUM(CASE WHEN ${deliveryBoys.currentAssignedOrders} > 0 THEN 1 ELSE 0 END)`,
      })
      .from(deliveryBoys);
