...
function broadcastToDeliveryBoys(event: string, data: any) {
  const message = `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
  for (const controllers of sseClients.values()) {
    for (const ctrl of controllers) {
      try { ctrl.enqueue(new TextEncoder().encode(message)); } catch {}
    }
  }
}
...
export const deliveryBoyLogin = os
  ...
  .handler(async ({ input }) => {
    ...
    if (!boy) return { success: false, error: 'Invalid phone or password' };
    if (!boy.isActive) return { success: false, error: 'Account is inactive' };

    const valid = await verifyPassword(input.password, boy.password);
    if (!valid) return { success: false, error: 'Invalid phone or password' };
...
export const getMyAssignedOrders = deliveryBoyProcedure
  .handler(async ({ context }) => {
    try {
      const { id } = (context as any).deliveryBoy;
...
export const claimOrder = deliveryBoyProcedure
  ...
  .handler(async ({ input: orderId, context }) => {
    const { id, name } = (context as any).deliveryBoy;

    // Check it's still unclaimed
    const [order] = await db.select().from(orders).where(eq(orders.id, orderId)).limit(1);
    if (!order) throw new Error('Order not found');
    if (order.deliveryBoyId) throw new Error('Order already claimed');
    ...
    await db.update(deliveryBoys).set({
      currentAssignedOrders: db.$count(orders, and(eq(orders.deliveryBoyId, id), eq(orders.status, 'out_for_delivery'))) as any,
      isAvailable: false,
    }).where(eq(deliveryBoys.id, id));
